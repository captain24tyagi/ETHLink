...
export default function Feed() {
  ...
  const getNews = () => {
    axios
      .get(
        'https://newsapi.org/v2/everything?q=bitcoin&apiKey=a85975e3c8e3499daa0ded6bf6050afe'
      )
      .then((response) => {
  ...
  const getNews1 = () => {
    axios
      .get(
        'https://newsapi.org/v2/everything?q=ai&apiKey=a85975e3c8e3499daa0ded6bf6050afe'
      )
      .then((response) => {
        setData(response.data.articles)
      })
  ...
  const getNews2 = () => {
    axios
      .get(
        'https://newsapi.org/v2/everything?q=cyber&apiKey=a85975e3c8e3499daa0ded6bf6050afe'
      )
      .then((response) => {
        setData(response.data.articles)
      })
  ...
  const getNews3 = () => {
    axios
      .get(
        'https://newsapi.org/v2/everything?q=cloud&apiKey=a85975e3c8e3499daa0ded6bf6050afe'
      )
      .then((response) => {
        setData(response.data.articles)
      })
  ...
  const getNews4 = () => {
    axios
      .get(
        'https://newsapi.org/v2/everything?q=iot&apiKey=a85975e3c8e3499daa0ded6bf6050afe'
      )
      .then((response) => {
        setData(response.data.articles)
      })
          ...
          <div className='bg-[#4D4D5D] my-5 rounded-lg h-[700px] overflow-y-scroll no-scrollbar'>
            ...
            <div className=' justify-between items-start py-3 my-2 rounded-lg bg-[#4D4C5D]'>
              <div className='flex flex-row text-center p-3 mx-3 space-x-3'>
                <button
                  onClick={getNews}
                  className='px-2 py-1 bg-black hover:bg-[#F18404] rounded-2xl'
                >
                  WEB3
                </button>
                <button
